...
class MainController @Inject()(
                               ...
                               @Named("messageHubActor") messageHubActor: ActorRef)
  ...
  def originMatches(origin: String): Boolean = {
    true//origin.contains("localhost:9000") || origin.contains("localhost:19001")
  }
  ...
  def wsFutureFlow(request: RequestHeader): Future[Flow[JsValue, JsValue, NotUsed]] = {
    ...
    val clientActorFuture = createClientActor(request.id.toString, webSocketOut)

    // Once we have an actor available, create a flow...
    clientActorFuture.map { clientActor =>
      createWebSocketFlow(webSocketIn, clientActor)
    }
  }
   ...
   *
   * @param clientActor   the user actor receiving websocket events.
   * @param webSocketIn the "read" side of the websocket, that publishes JsValue to clientActor.
   * @return a Flow of JsValue in both directions.
   */
  def createWebSocketFlow(webSocketIn: Publisher[JsValue], clientActor: ActorRef): Flow[JsValue, JsValue, NotUsed] = {
    ...
    val flow = {
      val sink = Sink.actorRef(clientActor, akka.actor.Status.Success(()))
    ...
    val flowWatch: Flow[JsValue, JsValue, NotUsed] = flow.watchTermination() { (_, termination) =>
      termination.foreach { done =>
        logger.info(s"Terminating actor $clientActor")
        messageHubActor.tell(MessageHubActor.Exit, clientActor)
   ...
   */
  def createClientActor(name: String, webSocketOut: ActorRef): Future[ActorRef] = {
    // Use guice assisted injection to instantiate and configure the child actor.
    val clientActorFuture = {
      implicit val timeout = Timeout(100.millis)
      val ref = (clientManagerActor ? ClientManagerActor.Create(name, webSocketOut))
      ref.mapTo[ActorRef]
    }
    clientActorFuture
  }
